...
function abrirModal(produto) {
    ...
    if (produto.ingredientes_disponiveis && produto.ingredientes_disponiveis.length > 0) {
        produto.ingredientes_disponiveis.forEach((ing, index) => {
            ...
            div.innerHTML = `
                <input type="checkbox" id="ing_${index}" value="${ing.nome}">
                <label for="ing_${index}">Remover ${ing.nome}</label>
            `;
            listaIngredientes.appendChild(div);
        });
    } else {
        listaIngredientes.innerHTML = "<p style='color:#888'>Sem modificações disponíveis.</p>";
    }
...
function adicionarAoCarrinho() {
    ...
    setTimeout(() => {
        atualizarCarrinho();
        // Feedback visual agradável em vez daquele modal enorme
...
function atualizarCarrinho() {
    ...
    carrinho.forEach((item, index) => {
        ...
        let textoRemovidos = item.removidos.length > 0 
            ? `<div class="item-ingredientes">Sem: ${item.removidos.join(', ')}</div>` 
            : '';
    ...
    if (carrinho.length === 0) {
        itensContainer.innerHTML = '<p style="text-align: center; color: #888; margin-top: 50px;">O seu carrinho está vazio.</p>';
        fecharCarrinho(); // Esconde a gaveta se esvaziar
    }

    document.getElementById("total").innerText = total.toFixed(2);

    // Atualiza o Botão Flutuante do Mobile
